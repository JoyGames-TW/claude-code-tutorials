# 04-1-2 重構精簡：消除過度設計與重複代碼

> ⚠️ **線上核實狀態**：已核實（2026-06-06）。本章關於程式碼簡化與重構的原則（DRY、YAGNI、可讀性優先）皆為軟體工程通用最佳實務，100% 正確。
> **注意**：`/simplify` 作為獨立 Slash Command 的可用性需以您的 Claude Code 版本為準。
> 即使沒有 `/simplify` 指令，本章的核心方法——「在 Prompt 中明確要求 Claude 識別重複模式、過度設計並提供重構建議」——完全有效。

## 1. 本章學習目標

- 學會使用 Claude Code 進行程式碼精簡與重構（無論是否有 `/simplify` 指令）
...
### 4.1 要求 Claude 進行程式碼簡化

**通用方法（所有 Claude Code 版本有效）**：直接在 Prompt 中要求簡化分析。

若您的版本支援 `/simplify` 指令，可先輸入 `/simplify` 再貼上以下 Prompt。若不支援，直接使用以下 Prompt 即可：
...
1. 程式碼簡化是消除過度設計與重複的品質活動，不是「把程式碼變短」
2. 簡化的信號：重複模式、只有一個實作的 Interface、過深的巢狀結構、過早的抽象
3. 簡化必須在測試保護下進行——先確保有測試，再進行簡化
4. 簡化的標準是「可讀性」與「可維護性」，不是「最少行數」
5. 小步簡化、頻繁 Commit、定期排定簡化時間
6. 無論是否有 `/simplify` 指令，結構化 Prompt 都能達到相同的重構效果
