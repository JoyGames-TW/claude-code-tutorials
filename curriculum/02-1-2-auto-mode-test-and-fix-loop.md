# 02-1-2 自動化開發：讓 Claude Code 自主跑完測試與修正迴圈

> ⚠️ **線上核實狀態**：已核實（2026-06-06）。自動化測試修正循環的概念正確，Maven 測試指令準確。
> **注意**：「Auto Mode」的具體觸發方式（指令名稱或設定方法）隨 Claude Code 版本而異。
> 本章的核心價值在於「定義清楚的停止條件與檢查點」策略——這在所有版本中都適用。
