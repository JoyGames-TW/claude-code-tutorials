# 04-2-1 常用指令開發時序表：從初始化到交付的指令應用時機

> ⚠️ **線上核實狀態**：已核實（2026-06-06）。開發生命週期中各階段的任務需求與指令對應關係正確。
> **注意**：部分 Slash Command 的可用性隨 Claude Code 版本而異（詳見各對應章節的核實狀態）。
> 本章的核心價值在於「開發階段的思維模型」——知道何時該做什麼，比記住特定指令名稱更重要。
...
- 查核日期：2026-06-06（已核實）
- 版本備註：Slash Commands 清單與行為以 Claude Code 最新版本為準；部分指令的可用性詳見各對應章節的核實狀態
