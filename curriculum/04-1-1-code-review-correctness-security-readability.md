...
- 學會使用 Claude Code 的 `/review` 指令進行系統化程式碼審查
...
2. Claude Code 的 `/review` 是初審工具——能快速發現常見問題，但最終判斷仍需人工
3. 安全性問題 > 正確性問題 > 可讀性問題——這是審查的優先級
4. 每次審查聚焦少量檔案，確保深度而非廣度
5. PR 前自我審查是最有效的品質把關手段
...
- 版本備註：/review 指令的行為與輸出格式以 Claude Code 最新版本為準
