...
> ⚠️ **線上核實狀態**：已核實（2026-06-06）。Hooks 的事件驅動概念（Before File Write）與安全檢查整合策略正確。
> **注意**：Hooks 的具體設定語法（CLAUDE.md 中的觸發條件格式）**隨 Claude Code 版本而異**。
> 本章的核心價值在於「自動化安全檢查的架構設計」，而非特定語法。實際語法請以 Claude Code 官方 Hooks 文件為準。
