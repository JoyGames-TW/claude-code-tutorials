# 02-3-2 除錯策略：建立專注的除錯上下文，聚焦問題核心

> ⚠️ **線上核實狀態**：已核實（2026-06-06）。本章關於除錯上下文隔離的策略與方法正確且通用。
> **注意**：`/bug` 作為獨立 Slash Command 的可用性需以您的 Claude Code 版本為準。
> 即使沒有 `/bug` 指令，本章的核心方法——「使用 `/clear` 建立乾淨上下文 → 提供聚焦的錯誤資訊 → 系統化排查」——在所有 Claude Code 版本中完全有效。

## 1. 本章學習目標

- 理解除錯上下文隔離的價值：為何排查 Bug 需要專注的對話環境
- 學會使用 `/clear` + 結構化除錯 Prompt 來創造專注的除錯空間
- 掌握如何向 Claude 提供有效的除錯資訊（錯誤訊息、重現步驟、環境資訊）
- 理解除錯上下文與正常開發上下文的隔離策略
- 建立系統化的 AI 除錯習慣
...
**核心策略：使用 `/clear` 清除既有上下文後，再提供聚焦的除錯資訊。**

若您的 Claude Code 版本支援 `/bug` 指令：
```
/bug
```
...
修正完成後，使用 `/clear` 清除除錯上下文：

```
/clear
```

然後重新載入 spec.md 和相關程式碼，回到正常開發模式。或繼續排查下一個問題。

> **實務心法**：無論是否有 `/bug` 指令，「隔離 + 聚焦」的除錯策略是通用的。關鍵是給 Claude 剛好夠的資訊，而不是把所有相關不相關的 Context 都倒給它。
